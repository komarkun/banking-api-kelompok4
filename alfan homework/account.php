...
function deleteAccount($akun_id)
{
    ...
    if ($delete_data->execute()) {
        echo json_encode(['message' => 'berhasil mneghapus akun']);
    } else {
        echo json_encode(['error' => 'akun gagal dihapus']);
    }
...
function addBalance($akun_id, $saldo)
{
    ...
    if (updateAccountBalance($akun_id, $saldo)) {
        echo json_encode(['message' => 'uang berhasil tersimpan']);
    } else {
        echo json_encode(['error' => 'transaksi gagal']);
    }
}
